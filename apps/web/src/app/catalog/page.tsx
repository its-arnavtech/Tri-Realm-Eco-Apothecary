...
export default async function CatalogPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  return <main id="main"><Analytics page="catalog" /><section className="page-hero"><div className="shell"><p className="section-number">THE APOTHECARY / 002</p><h1>Explore the <em>brews.</em></h1><p>Meet four early product concepts. Prices are illustrative and final formulations, instructions and claims remain under review.</p></div></section>
    <section className="content-section shell"><div className="catalog-toolbar"><div><p className="section-number">THE COLLECTION</p><p>{products.total} concept brew{products.total === 1 ? "" : "s"}</p></div>
      <form className="filter-form" action="/catalog"><div className="field"><label htmlFor="biome">Realm</label><select id="biome" name="biome" defaultValue={filters.biome ?? ""}><option value="">All realms</option><option value="forest">Forest</option><option value="ocean">Ocean</option><option value="mountain">Mountain</option><option value="tri-realm">Tri-Realm</option></select></div>
        <div className="field"><label htmlFor="product_type">Type</label><select id="product_type" name="product_type" defaultValue={filters.product_type ?? ""}><option value="">All types</option><option value="home-care concept">Home care</option><option value="multi-surface concept">Multi-surface</option></select></div>
        <div className="field"><label htmlFor="refillable">Refill concept</label><select id="refillable" name="refillable" defaultValue={filters.refillable ?? ""}><option value="">Any</option><option value="true">Yes</option><option value="false">No</option></select></div><button type="submit">Apply filters</button></form></div>
