...
export function ProductCard({ product }: { product: Product }) {
  return <article className={`product-card product-card--${product.biome}`}>
    ...
    <div className="product-card__body"><div className="eyebrow">Brew No. {product.brew_number} <span>·</span> {product.biome.replace("-", " ")}</div>
      ...
      <div className="product-card__bottom"><span>{product.availability_status === "available" ? "" : "Illustrative "}{money(product.price_cents)}</span><Link href={`/catalog/${product.slug}`} aria-label={`View details for ${product.name}`}>View brew <span aria-hidden="true">↗</span></Link></div>
    </div>
  </article>;
}
