...
4. Add one build environment variable: `ENABLE_EXPERIMENTAL_COREPACK=1`. This uses the `pnpm@10.32.1` version pinned in `apps/web/package.json`, which Vercel supports. Remove any old `API_INTERNAL_URL`, `DATABASE_URL`, or API-related variables from the web project; the static website does not use them.
